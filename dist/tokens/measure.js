...
import { buildDoneSessionPrompt, buildNextSessionPrompt, buildPlanSessionPrompt, } from '../prompts/sessionContext.js';
...
function measureSpec(spec, real) {
...
function phaseHistoryText(phases) {
    return phases.map((p) => `- [${p.status}] ${p.id}. ${p.title}`).join('\n');
}
/** Text bloku „fáze (název + cíl)". */
function phaseHeadText(phase) {
    return `${phase.title}\n${phase.goal ?? ''}`;
}
/** Text bloku „kroky". */
function stepsText(phase) {
...
const doneSpec = {
    id: 'done',
    build: (i) => buildDoneSessionPrompt({
        phase: i.phase,
        reportExists: true,
...
const writeMemorySpec = {
    id: 'writeMemory',
    build: (i) => buildWriteMemoryPrompt({
        projectMd: i.projectMd,
        phase: i.phase,
...
export function measureAll(input) {
    return [
...
export function rankMeasurements(measurements) {
    return [...measurements].sort((a, b) => b.realTokens - a.realTokens || a.command.localeCompare(b.command));
}
// --- Render ---------------------------------------------------------------
 ...
 */
export function renderReportMarkdown(ranked) {
    const lines = [];
...
export function renderConsole(ranked) {
    ...
    for (const m of ranked) {
        ...
        lines.push(`  ${name}  reálný=${m.realTokens}  šablona=${m.templateTokens}  kontext=${m.injectedTokens}  (${whyLine(m)})`);
    }
    return lines.join('\n');
}
