...
export function buildNextPhasePrompt(projectMd, state, optionsOrHint) {
    const options = typeof optionsOrHint === 'string' ? { userHint: optionsOrHint } : optionsOrHint ?? {};
    const userHint = options.userHint;
    const lastMemoryMd = options.lastMemoryMd;
    const historyLines = state.phases.map((phase) => `- [${PHASE_WORD[phase.status]}] ${phase.id}. ${phase.title}`);
