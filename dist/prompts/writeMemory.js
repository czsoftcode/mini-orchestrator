...
export function buildWriteMemoryPrompt(input) {
    ...
    const contextLines = [];
    if (hasAutoCommit) {
        contextLines.push('- Spusť `git show HEAD` přes Bash a podívej se na diff posledního commitu — to je práce této fáze.');
    }
    else {
        contextLines.push('- `git show HEAD` **nepouštěj** — buď tato fáze žádný commit nevytvořila, nebo nejsi v gitovém repu. Vyjdi jen z informací níže.');
    }
    if (discussPath) {
