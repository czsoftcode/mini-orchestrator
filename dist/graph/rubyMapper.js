 ...
 */
export function mapRubyFile(content, relPath) {
 ...
 */
function extractRubyImports(noComments) {
    const imports = [];
 ...
 */
function extractRubyExports(stripped) {
    const exports = [];
    const frames = [];
 ...
 */
function processStatement(stmtRaw, lineNo, lines, li, frames, exports) {
    let stmt = stmtRaw;
    let forcedVis;
...
function setVisibility(frames, vis) {
...
function nearestScope(frames) {
    for (let i = frames.length - 1; i >= 0; i--) {
        const f = frames[i];
        if (f && (f.isType || f.kind === 'def'))
            return f;
    }
    return undefined;
}
 ...
 */
function openType(stmt, keyword, lineNo, frames, exports) {
    ...
    if (name && isTopLevel) {
        const kind = keyword === 'class' ? 'class' : 'module';
        const exp = { name, kind, line: lineNo };
        const methods = [];
        exports.push(exp);
 ...
 */
function handleDef(stmt, lineNo, lines, li, frames, exports, forcedVis) {
    ...
    if (!scope) {
        // top-level funkce
        const exp = { name, kind: 'function', signature, line: lineNo };
        exports.push(exp);
        if (endless) {
            exp.endLine = endLi + 1;
        }
 ...
 */
function parseDefSignature(sigPart) {
 ...
 */
function parseRubyParams(paramsStr) {
    const result = [];
 ...
 */
function handleAttr(keyword, stmt, frames, forcedVis) {
    const scope = nearestScope(frames);
    if (!scope || !scope.isType || !scope.exp || !scope.methods)
        return;
    const vis = forcedVis ?? scope.visibility ?? 'public';
    if (vis !== 'public')
        return;
    ...
    for (const name of names) {
        if (reader)
            scope.methods.push({ name, signature: { parameters: [] } });
        if (writer) {
...
function closeFrame(frame, lineNo) {
    if (frame.exp) {
