 ...
 */
export function mapSwiftFile(content, relPath) {
...
function atWordBoundary(text, p) {
...
function skipAttribute(text, atPos) {
    let p = atPos + 1;
 ...
 */
function extractSwiftImports(stripped) {
    const imports = [];
    ...
    while ((m = regex.exec(stripped)) !== null) {
        ...
        if (kind !== null && IMPORT_KINDS.has(kind)) {
            ...
            if (wordAt(stripped, afterKind) !== null)
                p = afterKind;
        }
 ...
 */
function extractSwiftExports(stripped) {
    const exports = [];
 ...
 */
function findTypeBrace(text, from) {
    const n = text.length;
    let i = from;
    let paren = 0;
    let angle = 0;
    ...
    while (i < to) {
        ...
        if (decl) {
            if (decl.keyword === 'func') {
                ...
                if (fn) {
                    if (decl.visible) {
                        const method = { name: fn.name, signature: fn.signature };
                        if (decl.isStatic)
...
function parseFunctionAt(stripped, kwPos, declStart) {
    ...
    if (name === null)
        return null; // operátorová funkce
    p += name.length;
    p = skipWs(stripped, p);
 ...
 */
function parseSwiftParams(paramsStr) {
    const result = [];
