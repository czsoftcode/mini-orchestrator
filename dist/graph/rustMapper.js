 ...
 */
export function mapRustFile(content, relPath) {
...
function stripRustCommentsAndStrings(content) {
    const out = [];
    let i = 0;
    ...
    while (i < n) {
        ...
        if (c === "'") {
            ...
            if (charMatch) {
                for (let k = 0; k < charMatch[0].length; k++)
                    out.push(' ');
...
function isRawStringStart(content, pos) {
    let i = pos;
    // přeskoč případné prefixy br / cr / r
    if ((content[i] === 'b' || content[i] === 'c') && content[i + 1] === 'r')
        i += 2;
    else if (content[i] === 'r')
        i += 1;
    else
...
function consumeRawString(content, pos) {
    let i = pos;
    if ((content[i] === 'b' || content[i] === 'c') && content[i + 1] === 'r')
        i += 2;
    else
...
function consumeQuotedString(content, pos) {
    if (content[pos] !== '"')
        return 0;
    let i = pos + 1;
    while (i < content.length) {
 ...
 */
function itemEnd(stripped, fromPos) {
    for (let i = fromPos; i < stripped.length; i++) {
        const c = stripped[i];
        if (c === '{')
            return { endLine: lineAt(stripped, matchBrace(stripped, i)) };
        if (c === ';')
            return { endLine: lineAt(stripped, i) };
    }
    return {};
}
...
function matchAngle(stripped, openPos) {
    let depth = 0;
    for (let i = openPos; i < stripped.length; i++) {
        if (stripped[i] === '<')
            depth++;
...
function extractRustImports(stripped) {
    const imports = [];
...
function parseRustUseBody(body, out) {
    // Group form: prefix::{ ... }
...
function extractRustExports(stripped) {
    const exports = [];
...
function parseRustFnAfterName(stripped, fromPos) {
    let p = fromPos;
    // přeskoč whitespace
    while (p < stripped.length && isWhitespace(stripped[p]))
        p++;
    // generics <...>
    if (stripped[p] === '<') {
    ...
    if (retMatch && retMatch[1] !== undefined)
        sig.returnType = retMatch[1].trim();
    return sig;
}
