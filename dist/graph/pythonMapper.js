 ...
 */
export function mapPythonFile(content, relPath) {
 ...
 */
function matchStringStart(content, i) {
    const c = content[i];
    if (c === undefined)
        return null;
    if (c === '"' || c === "'") {
...
function stripPythonCommentsAndStrings(content) {
    const out = [];
    let i = 0;
    ...
    while (i < n) {
        ...
        if (s) {
            ...
            for (let k = 0; k < s.prefixLen; k++)
                out.push(content[i + k] ?? ' ');
            let j = i + s.prefixLen;
...
function endsWithBackslash(text) {
    return text.replace(/\s+$/, '').endsWith('\\');
}
function matchParen(content, openPos) {
    let depth = 0;
 ...
 * `headerIndent`.
 */
function suiteRange(content, lines, startIdx, headerIndent) {
...
function statementEndLine(lines, startIdx) {
    let depth = 0;
...
function gatherLogical(lines, startIdx) {
    const parts = [];
    let depth = 0;
    for (let idx = startIdx; idx < lines.length; idx++) {
        const ln = lines[idx];
        ...
        depth += bracketDelta(ln.text);
        if (depth <= 0 && !endsWithBackslash(ln.text))
            break;
    }
    return parts.join(' ');
}
...
function parseImportStatement(text, out) {
...
// Exporty
// ---------------------------------------------------------------------------
function isUpperConst(name) {
    return /^[A-Z_][A-Z0-9_]*$/.test(name) && /[A-Z]/.test(name);
}
function matchConst(trimmed) {
    ...
    if (asg && asg[1] !== undefined) {
        const name = asg[1];
        if (!name.startsWith('_') && isUpperConst(name))
            return { name };
    }
    return null;
}
    ...
    for (let idx = 0; idx < lines.length; idx++) {
        ...
        if (statementStart && !ln.blank && ln.indent === 0) {
            ...
            if (trimmed.startsWith('@')) {
                if (pendingDecoratorLine === null)
                    pendingDecoratorLine = ln.lineNo;
            }
            else {
 ...
 * vynechají.
 */
function extractMethods(content, lines, classRange, classIndent) {
    if (classRange.inline)
        return [];
    const methods = [];
    let depth = 0;
    let prevBackslash = false;
    ...
    for (let idx = classRange.colonLineNo; idx < lines.length; idx++) {
        const ln = lines[idx];
        if (ln.lineNo > classRange.endLine)
            break;
        const statementStart = depth === 0 && !prevBackslash;
        if (statementStart && !ln.blank) {
            if (bodyIndent === -1 && ln.indent > classIndent)
                bodyIndent = ln.indent;
            if (ln.indent === bodyIndent) {
                const trimmed = ln.text.trimStart();
                if (trimmed.startsWith('@')) {
                    if (/^@\s*staticmethod\b/.test(trimmed))
                        pendingStatic = true;
                }
                else {
                    const defM = /^(?:async\s+)?def\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(/.exec(trimmed);
...
function parsePythonSignature(content, openParenPos) {
