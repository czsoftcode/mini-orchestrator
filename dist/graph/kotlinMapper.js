 ...
 */
export function mapKotlinFile(content, relPath) {
...
function lastNonWsChar(text, from, low) {
    let i = from;
    while (i >= low && isWhitespace(text[i]))
        i--;
    return i >= low ? (text[i] ?? '') : '';
}
...
function skipAnnotation(text, atPos) {
    let p = atPos + 1;
    while (p < text.length && (isWordChar(text[p]) || text[p] === '.' || text[p] === ':'))
        p++;
    p = skipWs(text, p);
    if (text[p] === '(')
 ...
 */
function extractKotlinImports(stripped) {
    const imports = [];
...
function typeKindFor(decl) {
    if (decl.isEnum)
        return 'enum';
    if (decl.keyword === 'interface')
        return 'interface';
 ...
 */
function extractKotlinExports(stripped) {
    const exports = [];
...
function parseType(stripped, decl) {
    ...
    if (body.brace === -1) {
        ...
        if (!decl.visible)
            return { next: body.end, export: null };
        const exp = {
            name,
            kind,
 ...
 */
function findClassBody(text, from) {
    const n = text.length;
    let i = from;
    let paren = 0;
    let angle = 0;
 ...
 */
function parseTypeBody(stripped, from, to) {
    const methods = [];
    let i = from;
    let depth = 0;
    while (i < to) {
        ...
        if (decl) {
            if (decl.keyword === 'fun') {
                ...
                if (fn) {
                    if (decl.visible)
                        methods.push({ name: fn.name, signature: fn.signature });
                    i = Math.max(fn.next, i + 1);
                    continue;
                }
            }
 ...
 */
function parseFunctionAt(stripped, kwPos, declStart) {
...
function findParamParen(text, p) {
    let i = p;
    let angle = 0;
    while (i < text.length) {
        const c = text[i];
...
function identBefore(text, parenPos) {
    let i = parenPos - 1;
 ...
 */
function readReturnType(text, from) {
    const n = text.length;
    let i = from;
    let paren = 0;
    let angle = 0;
 ...
 * přítomnost `= ...` → `optional`.
 */
function parseKotlinParams(paramsStr) {
    const result = [];
