...
import { buildRenumberMap, executeRenames, findCollisions, planMemoryDir, planSimpleDir, } from '../state/renumber.js';
 ...
 */
export async function renumber(cwd = process.cwd(), opts = {}) {
    if (!(await exists(cwd))) {
    ...
    const totalRenames = planPhases.renames.length +
...
function printPreview(state, idMap, idChanges, plans) {
