...
import { RunReportParseError, readRunReport, runReportPath, } from '../state/runReport.js';
import { exists, load, save } from '../state/store.js';
import { ask } from '../ui/ask.js';
import { isInteractive } from '../ui/interactive.js';
import { log } from '../ui/log.js';
import { writePhaseMemory } from './writeMemory.js';
export async function done(opts = {}) {
    ...
    if (doingStep) {
        const moreStepsAfter = (phase.steps ?? []).some((s) => s !== doingStep && s.status !== 'done' && s.status !== 'skipped');
...
async function collectNotesAndSave(phase, state, cwd, outcome, opts = {}) {
    if (!opts.auto) {
 ...
 */
export function buildPhaseCommitMessage(phase) {
 ...
 */
async function finalizePhaseSideEffects(phase, state, cwd, finalizeOpts = {}) {
 ...
 * `/graphify`.
 */
async function regenerateGraph(cwd) {
    try {
        ...
        log.warn(`Mapu projektu se nepodařilo přegenerovat: ${err.message}`);
    }
}
/**
 ...
 */
async function commitPhaseWork(phase, cwd, finalizeOpts = {}) {
 ...
 */
async function tagVersion(cwd, version) {
    if (!version)
        return;
    const tag = `v${version}`;
 ...
 */
async function stampChangelog(cwd, version) {
    if (!version)
        return;
    ...
    try {
        ...
        log.warn(`Zápis ${CHANGELOG_FILE} selhal: ${err.message}`);
    }
}
/**
 ...
 */
async function bumpVersion(cwd, level) {
    try {
        ...
        log.warn(`Navýšení verze selhalo: ${err.message}`);
    }
    return null;
}
export function advanceToNextPhase(state) {
    closeOrphanedDoingParents(state);
    const nextPhase = state.phases.find((p) => p.id !== state.currentPhaseId && (p.status === 'proposed' || p.status === 'planned')) ?? null;
 ...
 */
function closeOrphanedDoingParents(state) {
    for (const parent of state.phases) {
        if (parent.status !== 'doing')
            continue;
        const subs = state.phases.filter((p) => p.id !== parent.id && Math.floor(p.id) === parent.id);
        if (subs.length === 0)
            continue;
        if (!subs.every((s) => s.status === 'done' || s.status === 'skipped'))
            continue;
        parent.status = 'done';
...
async function finalizeStep(step, moreStepsLeft, opts = {}) {
    let outcome;
    if (opts.auto) {
        outcome = 'done';
    }
...
export async function applyAutoReport(phase, state, cwd, applyOpts = {}) {
    ...
    const remaining = (phase.steps ?? []).filter((s) => s.status !== 'done' && s.status !== 'skipped');
    if (!phase.steps?.length) {
        ...
        log.warn(`Fáze ${phase.id}: zbývá ${remaining.length} ${remaining.length === 1 ? 'nedokončený krok' : 'nedokončených kroků'} (verdict: ${report.verdict}).`);
 ...
 * znovu nenabízejí.
 */
async function handleVerify(verify, phase, state, cwd, applyOpts = {}) {
    ...
    if (!isInteractive()) {
        const w = pending.length === 1 ? 'bod k ručnímu ověření' : 'bodů k ručnímu ověření';
        log.warn(`Fáze ${phase.id} (${phase.title}): ${pending.length} ${w} vyžaduje člověka, ale běžím bez interaktivního terminálu — fázi nezavírám.`);
    ...
    if (blockers.length > 0) {
        ...
        log.warn(`Fáze ${phase.id} se neuzavírá — našel se ${blockers.length === 1 ? 'bloker' : 'blokery'}. Vytvořil jsem opravnou podfázi ${sub.id}.`);
    ...
    if (issues.length > 0) {
        log.warn(`Fáze ${phase.id} se neuzavírá — ${issues.length === 1 ? 'bod má problém' : 'bodů má problém'}:`);
 ...
 */
function insertFixSubphase(state, parent, blockers) {
    const id = nextSubphaseId(state, parent.id);
    const steps = blockers.map((b) => ({
        title: b.title,
        status: 'todo',
 ...
 */
function nextSubphaseId(state, parentId) {
    const subs = state.phases.filter((p) => Math.floor(p.id) === parentId && p.id !== parentId);
    const base = subs.length === 0 ? parentId : Math.max(...subs.map((p) => p.id));
    return Math.round((base + 0.1) * 10) / 10;
}
async function finalizePhase(phase, state, cwd, opts = {}) {
    let outcome;
    if (opts.auto) {
        outcome = 'done';
    }
 ...
 */
export async function applyDone(cwd = process.cwd(), opts = {}) {
    if (!(await exists(cwd))) {
        log.warn('V tomto adresáři není projekt.');
        log.hint('Začni: mini init');
        return { ok: false, reason: 'no-project' };
