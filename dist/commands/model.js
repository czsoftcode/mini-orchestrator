import { MODEL_SCOPES, SCOPE_LABELS } from '../state/models.js';
...
const SCOPE_SET = new Set(MODEL_SCOPES);
export async function model(arg1, arg2) {
...
async function interactivePicker(state, cwd) {
...
async function interactiveModelForScope(state, scope, cwd) {
...
async function setScope(state, scope, value, cwd) {
    const v = value.trim();
...
function setScopeRaw(state, scope, value) {
    if (!state.models) {
        state.models = {};
    }
...
function unsetScope(state, scope) {
    if (state.models) {
