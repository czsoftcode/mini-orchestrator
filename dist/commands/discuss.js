...
export async function discuss() {
    ...
    try {
        ...
        log.error(`Nepodařilo se vytvořit adresář ${DISCUSS_DIR}: ${err.message}`);
    ...
    try {
        ...
        log.error(`Claude se nepodařilo spustit: ${err.message}`);
...
async function offerPhaseEdit(phase, cwd) {
    ...
    try {
        ...
        log.error(`Stav se nepodařilo uložit: ${err.message}`);
        return;
    }
