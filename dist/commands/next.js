 ...
 */
export async function applyNewPhase(title, goal, cwd = process.cwd()) {
    if (!(await exists(cwd))) {
        log.warn('V tomto adresáři není projekt.');
        log.hint('Začni: mini init');
        return { ok: false, reason: 'no-project' };
...
async function addPhaseManually(state, cwd, opts) {
    const { title, goal } = await ask([
        {
            type: 'text',
            name: 'title',
...
async function commitPhase(state, cwd, title, goal, opts) {
