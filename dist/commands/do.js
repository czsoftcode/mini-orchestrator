...
export async function doPhase(opts = {}) {
 ...
 */
export async function applyDoStart(cwd = process.cwd()) {
    if (!(await exists(cwd))) {
        log.warn('V tomto adresáři není projekt.');
        log.hint('Začni: mini init');
        return { ok: false, reason: 'no-project' };
 ...
 */
function findStepByTitle(steps, title) {
 ...
 */
export async function applyStepDone(title, cwd = process.cwd()) {
    if (!(await exists(cwd))) {
        log.warn('V tomto adresáři není projekt.');
        log.hint('Začni: mini init');
        return { ok: false, reason: 'no-project' };
