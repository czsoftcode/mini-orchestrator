import { buildGraph, GRAPH_DIR, GRAPH_INDEX, hasMappableProject, updateGraphFile, } from '../graph/buildGraph.js';
 ...
 * v Claude session.
 */
export async function map(files) {
    ...
    if (!(await hasMappableProject(cwd))) {
        log.warn('V projektu nejsou žádné mapovatelné soubory (.ts, .tsx, .php, .rs, .py, .go, .java, .cs, .kt, .kts, .swift, .rb).');
 ...
 */
async function mapFiles(cwd, files) {
    let updated = 0;
    let removed = 0;
    let skipped = 0;
    let fellBack = false;
    for (const file of files) {
        try {
            ...
            log.warn(`Nepodařilo se přemapovat ${file}: ${err.message}`);
        }
    }
    ...
    const parts = [];
    if (updated > 0)
        parts.push(`${updated} aktualizováno`);
    if (removed > 0)
        parts.push(`${removed} odebráno`);
    if (skipped > 0)
        parts.push(`${skipped} přeskočeno`);
