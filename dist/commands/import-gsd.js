...
export async function importGsd() {
    ...
    console.log();
    console.log(`  Fází: ${parsed.phases.length} (hotové: ${counts.done}, dělají se: ${counts.doing}, čekají: ${counts.proposed}, odloženo: ${counts.skipped})`);
    console.log();
    console.log('  Náhled:');
...
`;
    ...
    state.phases = parsed.phases.map((p, i) => ({
        id: i + 1,
        title: p.title,
        status: p.status,
    }));
