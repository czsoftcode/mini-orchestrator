 ...
 */
export async function syncSkeleton(cwd = process.cwd(), { dryRun = false } = {}) {
 ...
 */
export async function update(cwd = process.cwd(), { dryRun = false } = {}) {
    if (!(await exists(cwd))) {
    ...
    const changed = skel.createdDirs + skel.createdFiles + skel.updatedFiles + cmds.created + cmds.updated;
    console.log();
    if (changed === 0) {
