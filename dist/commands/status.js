...
function printPhase(p, isCurrent, state, summary) {
...
export function describeModels(state) {
    const parts = [];
 ...
 */
export function runReportSummaryLines(summary, phase) {
 ...
 */
export function isOrphanedDoing(phase, phases) {
...
function orphanedDoingNote(phase, phases) {
    const subs = phases.filter((p) => p.id !== phase.id && Math.floor(p.id) === phase.id);
    if (subs.length > 0) {
