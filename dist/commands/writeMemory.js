...
import { buildWriteMemoryPrompt, LAST_MEMORY_FILE, MEMORY_DIR, } from '../prompts/writeMemory.js';
 ...
 */
export async function writePhaseMemory(phase, state, cwd, options) {
    const memoryDirAbs = join(cwd, MEMORY_DIR);
    try {
        await mkdir(memoryDirAbs, { recursive: true });
    }
    catch (err) {
        log.warn(`Memory pro fázi ${phase.id} se nepodařilo zapsat: nemohu vytvořit ${MEMORY_DIR} (${err.message}).`);
        return;
    }
    ...
    if (state.models?.memory != null) {
        ...
        try {
            ...
            log.warn(`Memory pro fázi ${phase.id} se nepodařilo zapsat: ${err.message}`);
 ...
 */
export function buildPhaseMemoryMarkdown(phase, discussContent, runContent) {
    const parts = [];
 ...
 */
export function summarizeMemoryForNext(md) {
    ...
    const headEnd = Math.min(discussIdx === -1 ? text.length : discussIdx, runIdx === -1 ? text.length : runIdx);
    const parts = [text.slice(0, headEnd).trimEnd()];
    if (discussIdx !== -1) {
...
function indexOfSection(text, name, fromIndex) {
    const heading = `## ${name}`;
    if (fromIndex === 0 && text.startsWith(heading))
        return 0;
 ...
 */
function extractSubsection(block, matches) {
 ...
 */
async function writeViaClaude(phase, state, cwd, ctx) {
 ...
 * na disku.
 */
async function writeLastMemorySummary(cwd, memoryPathAbs, memoryPathRel) {
    ...
    try {
        ...
        if (err.code !== 'ENOENT') {
            log.dim(`(starý ${LAST_MEMORY_FILE} se nepodařilo smazat: ${err.message})`);
        }
    }
    try {
        ...
        log.dim(`(${LAST_MEMORY_FILE} se nepodařilo aktualizovat: ${err.message})`);
    }
}
 ...
 */
export async function freeMemoryFileName(dirAbs, phaseId) {
    const stem = phaseStem(phaseId);
    const base = `${stem}.md`;
    if (!(await fileExists(join(dirAbs, base))))
        return base;
    for (let n = 2;; n++) {
        const candidate = `${stem}-${n}.md`;
        if (!(await fileExists(join(dirAbs, candidate))))
            return candidate;
    }
}
