# Phase 110 — Status line opt-in při auto instalaci

**Goal:** In the auto (non-TTY global install) branch of runPostinstall, stop silently editing ~/.claude/settings.json — the status line becomes opt-in — and print an honest summary of which slash commands were created plus a one-line full-removal hint.

## Steps
- [done] Drop silent status-line write in auto branch
- [done] Print honest install summary + removal hint
- [done] Update postinstall tests for opt-in status line
- [done] Verify: typecheck, tests, simulated global install
