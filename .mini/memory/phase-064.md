# Fáze 64 — Inkrementální update grafu po editaci

**Cíl:** Přidat mini map --file <cesta>, který přemapuje jen jeden soubor (uzel v .mini/graph/ + jeho záznam v graph.json se zachováním pořadí, atomicky přes tmp+rename) místo plného rebuildu, a zapojit ho přes PostToolUse hook na Edit/Write s filtrem mapovatelných přípon a no-opem na ostatní; plný mini map zůstává jako fallback pro změny mimo agenta (mazání, rename, ruční editace, chybějící index).

## Kroky
- [hotovo] updateGraphFile v buildGraph.ts
- [hotovo] CLI flag map --file <cesty...>
- [hotovo] commands/map.ts podpora --file
- [hotovo] PostToolUse hook na Edit/Write
- [hotovo] Testy

## Auto-commit
- Fáze 64: Inkrementální update grafu po editaci (`97b45eab92dbebfb659c60c17812e6dc662a6d43`)
