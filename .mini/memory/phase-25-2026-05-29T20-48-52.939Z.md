# Fáze 25 — Bod 3. z .mini/improvements.md

**Cíl:** Fáze R1 - R4 budou vyřízené

## Kroky
- [hotovo] Propojit `--max-turns` z cli do `doPhase` (R1)
- [hotovo] Test ověří, že `--max-turns` dorazí do Claude (R1)
- [hotovo] Sjednotit ENOENT hlášku „claude" s návodem na instalaci (R3)
- [hotovo] Přidat `mini next` jeden retry při parse-failed (R4)
- [hotovo] Test ověří retry i tolerantní formát v `next` (R4)
- [hotovo] E2e test auto smyčky proti fake claude binárce (R2)
- [hotovo] `tsc --noEmit` čistý a všechny testy zelené
