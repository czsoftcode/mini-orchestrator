# Phase 181 — Doctor survives corrupt phase files

**Goal:** mini doctor catches CorruptPhaseError from load() and reports it as a separate diagnostic finding (with the corrupt file path) instead of crashing, with a test for a corrupt phase file

## Steps
- [done] Catch CorruptPhaseError around doctor's load
- [done] Render corruption as a failed check
- [done] Unit-test the corrupt-phase diagnostic
- [done] Integration test: doctor survives corruption
