...
- [ ] Adversarial findings: record HEAD SHA at review time as metadata (reviewed-at: <sha>) inside each finding — keep phase as the primary key, just stamp which code state was reviewed. Additive to findingsStore (optional field), surfaced in mini findings list. Lets a later consumer judge whether a finding is still relevant after the code moved on.
