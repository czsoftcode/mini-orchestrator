...
Jako úplně první věc si přečti \`.mini/graph.json\` (Read) — je to index strojové mapy projektu: pro každý zdrojový soubor jeho cestu, cestu k jeho mapě v \`.mini/graph/\` a názvy exportů. Podle exportů poznáš, který soubor co řeší, a **cíleně** si pak přečteš jen relevantní mapy z \`.mini/graph/<cesta>.md\` (exporty, importy, signatury) — nečti celý adresář najednou. Teprve když mapy nestačí, otevírej jednotlivé zdrojové soubory (Read, Grep, Glob). Kromě souboru s poznámkami (viz níže) nic jiného nezapisuj — session je jinak jen pro diskusi.
