 ...
 */
export function mapGoFile(content: string, relPath: string): FileGraph {
 ...
 */
function stripGo(content: string, blankStrings: boolean): string {
  const out: string[] = [];
  let i = 0;
  ...
  while (i < n) {
      ...
      while (j < n) {
        if (content[j] === '\\' && j + 1 < n) {
          if (blankStrings) {
            ...
            out.push(content[j] as string, content[j + 1] as string);
          }
          j += 2;
          continue;
        }
    ...
    // rune literál 'x' / '\n' / 'ሴ'
    if (c === "'") {
      const m = /^'(?:\\(?:x[0-9A-Fa-f]{2}|u[0-9A-Fa-f]{4}|U[0-9A-Fa-f]{8}|.)|[^'\\])'/.exec(
        content.slice(i),
      );
      if (m) {
        for (let k = 0; k < m[0].length; k++) out.push(blankStrings ? ' ' : (m[0][k] as string));
        i += m[0].length;
        continue;
      }
    }
...
function matchBracket(text: string, openPos: number): number {
  return matchPair(text, openPos, '[', ']');
}
...
function precedingWord(text: string, pos: number): string {
  let j = pos - 1;
 ...
 *  - `. "path"` → dot import (`['*']`)
 */
function extractGoImports(noComments: string): ImportInfo[] {
...
function parseImportSpec(spec: string, out: ImportInfo[]): void {
  ...
  let symbols: string[];
  if (alias === '_') symbols = [];
  else if (alias === '.') symbols = ['*'];
  else if (alias) symbols = [alias];
  else symbols = [last];
...
const KEYWORDS = ['func', 'type', 'const', 'var'] as const;
 ...
 */
function extractGoExports(stripped: string): ExportInfo[] {
  const exports: ExportInfo[] = [];
  const typeMap = new Map<string, ExportInfo>();
  ...
  while (i < n) {
    ...
    if (kw === 'func') {
      ...
      for (const e of r.exports) exports.push(e);
      for (const mth of r.methods) methods.push(mth);
...
function keywordAt(text: string, i: number): (typeof KEYWORDS)[number] | null {
  if (i > 0 && isWordChar(text[i - 1])) return null;
  for (const kw of KEYWORDS) {
    if (text.startsWith(kw, i) && !isWordChar(text[i + kw.length])) return kw;
  }
  return null;
}
...
function parseFunc(stripped: string, kwPos: number): FuncResult {
  ...
  if (!nameMatch || nameMatch[1] === undefined) return { next: p, exports: [], methods: [] };
  const name = nameMatch[1];
  p = skipWs(stripped, p + name.length);

  // Volitelné generické parametry funkce: func F[T any](...)
  if (stripped[p] === '[') p = skipWs(stripped, matchBracket(stripped, p) + 1);

  if (stripped[p] !== '(') return { next: p, exports: [], methods: [] };
  const paramsOpen = p;
  ...
  if (isExported(name)) {
    const exp: ExportInfo = { name, kind: 'function', signature: sig, line: lineAt(stripped, kwPos) };
    if (endLine !== undefined) exp.endLine = endLine;
...
function parseReceiverType(inner: string): string {
 ...
 */
function buildSignature(stripped: string, paramsOpen: number, paramsClose: number): FunctionSignature {
 ...
 */
function findBodyBrace(stripped: string, fromPos: number): number {
  let i = fromPos;
 ...
 */
function parseGoParams(paramsStr: string): Parameter[] {
  ...
  for (const seg of splitTopLevelCommas(paramsStr)) {
    ...
    for (const name of pending) result.push({ name, type: r.type });
    pending = [];
    const param: Parameter = { name: r.name };
    if (r.type) param.type = r.type;
    if (r.rest) param.rest = true;
    result.push(param);
  }
  for (const t of pending) result.push({ name: '_', type: t });
  return result;
}

function splitNameType(
  seg: string,
): { single?: string; name: string; type?: string; rest?: boolean } {
  const s = seg.trim();
  ...
  m = /^\.\.\.(.+)$/.exec(s);
  if (m && m[1] !== undefined) return { name: '_', type: m[1].trim(), rest: true };
  m = /^([A-Za-z_]\w*)\s+(.+)$/.exec(s);
  if (m && m[1] !== undefined && m[2] !== undefined) return { name: m[1], type: m[2].trim() };
...
function parseType(stripped: string, kwPos: number): DeclResult {
 ...
 */
function parseOneType(stripped: string, namePos: number, anchorPos: number): DeclResult {
  ...
  if (stripped[p] === '[') p = skipWs(stripped, matchBracket(stripped, p) + 1); // generika
  if (stripped[p] === '=') p = skipWs(stripped, p + 1); // alias `type T = X`
  ...
  if (/^struct\b/.test(rest) || /^interface\b/.test(rest)) {
    ...
    if (brace === -1) return { next: endOfLine(stripped, p), exports: [] };
    const close = matchBrace(stripped, brace);
    const exports: ExportInfo[] = exported
...
function parseConstVar(stripped: string, kwPos: number, kw: 'const' | 'var'): DeclResult {
  const kind: 'const' | 'variable' = kw === 'const' ? 'const' : 'variable';
...
function collectDeclNames(
  ...
  for (const raw of m[1].split(',')) {
    const name = raw.trim();
    if (name && isExported(name)) out.push({ name, kind, line: lineAt(stripped, anchorPos) });
  }
}
