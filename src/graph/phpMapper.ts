...
function extractPhpImports(stripped: string): ImportInfo[] {
...
function parsePhpUseBody(body: string, out: ImportInfo[]): void {
  // Group: Foo\Bar\{ ... }
  const groupMatch = /^([A-Za-z_\\][A-Za-z0-9_\\]*)\\\{([\s\S]*)\}$/.exec(body);
  if (groupMatch) {
    const prefix = groupMatch[1];
    const inner = groupMatch[2];
    if (prefix === undefined || inner === undefined) return;
    ...
    for (const raw of splitTopLevelCommas(inner)) {
      ...
      if (aliasMatch && aliasMatch[2] !== undefined) {
        ...
        if (!simple || simple[1] === undefined) continue;
...
function extractPhpExports(stripped: string): ExportInfo[] {
...
function findOpenBrace(stripped: string, fromPos: number): number {
  ...
  while (i < stripped.length) {
    const c = stripped[i];
    if (c === '{') return i;
    if (c === ';') return -1; // class deklarace bez těla? V PHP se nestane, ale buďme opatrní.
    i++;
  }
  return -1;
}

function extractPhpMethods(classBody: string): MethodSignature[] {
  ...
  while ((m = regex.exec(classBody)) !== null) {
    if (depthAt(classBody, m.index) !== 0) continue;
    const name = m[2] ?? m[3];
    if (!name) continue;
...
function findMethodHeadStart(text: string, fnPos: number): number {
  ...
  while (true) {
    let j = start - 1;
    while (j >= 0 && isMethodHeadSpace(text[j])) j--;
    const wordEnd = j + 1;
...
function isMethodHeadSpace(c: string | undefined): boolean {
  return c === ' ' || c === '\t' || c === '\n';
}
...
function parsePhpSignature(stripped: string, openParenPos: number): FunctionSignature {
  ...
  for (const raw of splitTopLevelCommas(paramsStr)) {
    ...
    if (hasDefault) param.optional = true;
  ...
  if (ret && ret[1] !== undefined) sig.returnType = ret[1];
  return sig;
}
