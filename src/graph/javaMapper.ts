 ...
 */
export function mapJavaFile(content: string, relPath: string): FileGraph {
 ...
 */
function stripJava(content: string): string {
  const out: string[] = [];
  let i = 0;
 ...
 */
function extractJavaImports(stripped: string): ImportInfo[] {
...
const TYPE_KEYWORDS = ['class', 'interface', 'enum', 'record'] as const;
type TypeKeyword = (typeof TYPE_KEYWORDS)[number];

const MEMBER_VISIBILITY = ['public', 'protected'] as const;

/** Mapuje deklarační klíčové slovo na `ExportKind`. */
function typeKindFor(kw: TypeKeyword | 'annotation'): ExportKind {
  switch (kw) {
    case 'class':
    case 'record':
      return 'class';
 ...
 */
function extractJavaExports(stripped: string): ExportInfo[] {
...
interface TypeDecl {
  /** Pozice začátku deklarace (první modifikátor / `@interface`). */
  declStart: number;
  /** Pozice klíčového slova (`class` / `interface` / `enum` / `record`). */
  kwPos: number;
  kw: TypeKeyword | 'annotation';
  visible: boolean;
}
...
function typeDeclAt(text: string, i: number): TypeDecl | null {
  ...
  while (p < text.length) {
    ...
    if ((TYPE_KEYWORDS as readonly string[]).includes(word)) {
      return { declStart, kwPos: p, kw: word as TypeKeyword, visible };
    }
...
function parseType(stripped: string, decl: TypeDecl): TypeResult {
  ...
  if (name === null) return { next: stripped.indexOf('\n', p) + 1 || stripped.length, export: null };
  p += name.length;
...
function parseMember(
  ...
  for (;;) {
    ...
    if (word === null) break;
    if (MEMBER_VISIBILITY.includes(word as (typeof MEMBER_VISIBILITY)[number])) {
      visible = true;
      p += word.length;
      continue;
    }
  ...
  if (implicitPublic && !hasPrivate) visible = true;
  ...
  if (decl.kind === 'method' && decl.namePos !== undefined && decl.parenPos !== undefined) {
    ...
    const method: MethodSignature = { name, signature: sig };
...
function scanToParenOrSemi(stripped: string, pos: number, limit: number): ScanResult {
  ...
  while (i < limit) {
    ...
    if (isWordChar(c)) {
      lastWordStart = i;
      while (i < limit && isWordChar(stripped[i])) i++;
      continue;
    }
    i++;
  }
 ...
 */
function parseJavaParams(paramsStr: string): Parameter[] {
  ...
  for (const seg of splitTopLevelCommas(paramsStr)) {
    ...
    if (m && m[1] !== undefined && m[2] !== undefined) {
      const param: Parameter = { name: m[2], type: m[1].trim() };
