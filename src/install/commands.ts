...
- a **stop hook** fired.`,
  },
  {
    ...
    body: `This is the **undo** step of the mini workflow, run directly in Claude Code. It reverts the last state change by one step. Mini remembers only **one** step back, so this is a one-shot, irreversible action — confirm it with the user before applying.
...
Never run a bare \`mini undo\` — it would block on an interactive Y/n prompt in this non-interactive Bash. Change the state in \`.mini/\` only via these commands, never edit \`.mini/state.json\` by hand.`,
  },
  {
    ...
    argumentHint: '[show | reset | <scope> <model>]',
    body: `This is the **model** step of the mini workflow, run directly in Claude Code. It views and sets the Claude model used for the project's mini steps. The model is stored in \`.mini/state.json\` — change it only via the \`mini model\` commands below, never by hand.
...
- **\`edit <n> <text>\`** → run \`mini todo edit <n> "<text>"\` to rewrite item number \`n\`'s text (keeps its done state).
- **\`done <n>\`** → run \`mini todo done <n>\` to tick item number \`n\` off.
- **\`remove <n>\`** (or \`rm <n>\`) → run \`mini todo remove <n>\` to drop item number \`n\`.
- **\`clear\`** → run \`mini todo clear\` to drop all done items at once (housekeeping).
