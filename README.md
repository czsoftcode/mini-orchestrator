...
| `mini doctor` | Health check of the project setup: state and schema version, `project.md`/`CHANGELOG.md` presence, installed slash-command count and mini version freshness — each line ok/warn/fail with a fix hint. Read-only |
...
| `mini completion <bash\|zsh>` | Prints a shell completion script that completes `mini`'s subcommands and each command's flags (e.g. `mini done --`+Tab). Enable with `source <(mini completion bash)` in your shell rc — see [`docs/non-interactive/completion.md`](docs/non-interactive/completion.md) |
